...
def login():
    ...
    if not login_result["success"]:
        return render_template(
            "/users/login.j2", form=form, error_msg=login_result["error_msg"]
        )
...
def register():
    ...
    if not form.validate_on_submit():
        return render_template("/users/register.j2", form=form)

    register_result = UserService.register(form)
    if register_result["success"] is False:
        return render_template(
            "/users/register.j2", form=form, error_msg=register_result["error_msg"]
        )
